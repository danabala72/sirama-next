...
import { calculateThreeAssessorScore, type AssessorScore, type ReportKind } from "./scores";
 ...
 */
export async function buildAssessmentReport(studentId: bigint, kind: ReportKind) {
  ...
  const assessorIds = student.asesorLinks.map((link) => link.asesorId).slice(0, 3);
  const assessorNames = student.asesorLinks.map((link) => link.asesor.name).slice(0, 3);

  const rows: AssessmentReportRow[] = student.mataKuliahPilihan.map((course) => {
    const scores: AssessorScore[] = assessorIds.map((assessorId) => {
      ...
      return { assessorId, formal: formal?.assessment?.score ?? null, nonformal: nonformal?.assessment?.score ?? null };
    });

    const result = calculateThreeAssessorScore(assessorIds, scores, kind);
    const formalSummary = selectCanonicalTransfer(course.transferSks.map((transfer) => ({
      id: transfer.id,
    ...
    const nonformalSummary = selectCanonicalTransfer(course.transferNonformal.map((transfer) => ({
      id: transfer.id,
    ...
    return {
      ...
      selfScore: course.nilaiAngka === null ? null : Number(course.nilaiAngka),
      assessorScores: result.assessorScores,
      average: result.average,
      complete: result.complete,
      duplicateTransfers: (formalSummary?.duplicateCount ?? 0) + (nonformalSummary?.duplicateCount ?? 0),
      conflictingScores: Boolean(formalSummary?.hasConflictingScores || nonformalSummary?.hasConflictingScores),
    };
  });

  return {
    student: { id: student.id.toString(), nim: student.nim, name: student.name },
