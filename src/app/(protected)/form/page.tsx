...
export default async function Page({ searchParams }: { searchParams: Promise<{ step?: string }> }) {
  ...
  const [attachments, courses, offerings] = await Promise.all([
    ...
    prisma.mataKuliahSemester.findMany({
      where: {
        mataKuliah: {
          jurusanId: student.user.jurusanId!, status: true,
          ...(student.user.skemaId ? { OR: [{ skema: { none: {} } }, { skema: { some: { skemaId: student.user.skemaId } } }] } : {}),
        },
  ...
  return <>
    ...
    {step === 6 && <div className="grid gap-5"><form action={saveStep6} className="grid gap-4">{courses.map((course) => { const transfer = course.transferSks[0]; const itemCount = Math.max(1, course.mataKuliahSemester?.capaian.length ?? 1, transfer?.cpmkItems.length ?? 0); return <section key={course.id.toString()} className="grid gap-3 rounded-lg border bg-white p-5 md:grid-cols-2"><h2 className="font-bold md:col-span-2">{course.kodeMk} — {course.namaMk}</h2><Field label="Kode MK asal" name={`code_${course.id}`} value={transfer?.kodeMkAsal ?? course.kodeMk} required /><Field label="Nama MK asal" name={`name_${course.id}`} value={transfer?.namaMkAsal ?? course.namaMk} required /><div className="md:col-span-2"><div className="mb-2 text-sm font-semibold">CPMK mata kuliah asal *</div>{Array.from({ length: itemCount }, (_, index) => <textarea key={index} className={`${inputClass} mb-2`} name={`cpmk_${course.id}`} defaultValue={transfer?.cpmkItems[index]?.cpmk ?? ""} placeholder={`CPMK asal ${index + 1}`} required />)}</div></section>; })}{!locked && courses.length > 0 && <button className={buttonClass}>Simpan Transfer SKS</button>}</form><FinalizeStudentForm enabled={!locked && completion.every(Boolean)} /></div>}
  </>;
}
